# Documentation

- [Installation](installation.md)
- [Architecture and backend identity](architecture.md)
- [Training and checkpoint resume](training.md)
- [Evaluation and benchmarking](evaluation.md)
- [Inference and export](inference.md)
- [Experiment reproducibility](reproducibility.md)
- [Core APIs](core.md)
- [Repository audit](repository_audit.md)
- [Without-Mamba parameter-count audit](without_mamba_parameter_audit.md)
- [PeerJ archival release readiness audit](peerj_release_readiness_audit.md)
- [Optimization plan](optimization_plan.md)

The reproducibility guide is the entry point for the completed baseline,
Phase 1-3, ablation, benchmark, and publication-analysis artifacts. Generated
research outputs remain outside version control by design.
