...
def compute_confusion_matrix(predictions: Tensor, targets: Tensor, num_classes: int) -> Tensor:
    ...
    predicted = predictions.argmax(dim=1) if predictions.ndim > targets.ndim else predictions
    ...
    valid = (actual >= 0) & (actual < num_classes) & (predicted >= 0) & (predicted < num_classes)
