...
class MultiScaleRestorationLoss(nn.Module):
    ...
    def __init__(self, weights: QRLossWeights | None = None, reconstruction: str = "charbonnier", perceptual: nn.Module | None = None) -> None:
        super().__init__()
        self.weights = weights or QRLossWeights()
        self.reconstruction = CharbonnierLoss() if reconstruction == "charbonnier" else ReconstructionLoss("l1")
    ...
    def forward(self, predictions: Tensor, targets: Tensor, metadata: list[dict[str, Any]] | None = None) -> Tensor:
        components = {"reconstruction": self.reconstruction(predictions, targets), "ssim": self.ssim(predictions, targets), "edge": self.edge(predictions, targets), "structure": self.structure(predictions, targets, metadata), "decode_consistency": self.decode(predictions, targets)}
        ...
        total = sum(getattr(self.weights, f"{name}_weight") * value for name, value in components.items())
