...
def generate_figures(rows: list[dict[str, Any]], directory: Path) -> list[Path]:
    ...
    try:
        ...
        raise ImportError(
            "Figures require matplotlib; install a plotting dependency."
        ) from error
    directory.mkdir(parents=True, exist_ok=True)
    paths: list[Path] = []
    for x, y, name in (
        ("degradation_severity", "psnr", "psnr_by_severity"),
        (
            "model_inference_latency_ms",
            "combined_decode_success_rate",
            "latency_vs_decode_rate",
        ),
    ):
        values = [
            (row.get(x), row.get(y))
        ...
        plt.close(figure)
        paths.append(directory / f"{name}.pdf")
    return paths
