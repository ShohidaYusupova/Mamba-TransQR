...
def aggregate(rows: list[dict[str, Any]], keys: tuple[str, ...]) -> list[dict[str, Any]]:
    ...
    for row in rows: groups.setdefault(tuple(row.get(key) for key in keys), []).append(row)
    output: list[dict[str, Any]] = []
    for group, values in groups.items():
        result = dict(zip(keys, group, strict=True)); numeric = {key for row in values for key, value in row.items() if isinstance(value, (int, float))}
        result.update({key: sum(float(row[key]) for row in values if isinstance(row.get(key), (int, float))) / sum(isinstance(row.get(key), (int, float)) for row in values) for key in numeric})
