...
class BaselineRegistry:
    ...
    def __init__(self) -> None:
        self._items: dict[str, BaselineSpec] = {}

    def register(self, spec: BaselineSpec) -> None:
        if spec.name not in self.names:
            raise ValueError(f"unknown baseline: {spec.name}")
        ...
        if spec.checkpoint is not None and not Path(spec.checkpoint).is_file():
            raise FileNotFoundError(
                f"declared checkpoint does not exist: {spec.checkpoint}"
            )
    ...
    def build(self, name: str) -> nn.Module:
        try:
            ...
            raise KeyError(f"baseline is not registered: {name}") from error
        if spec.factory is None:
            raise RuntimeError(
                f"{name} has no local implementation/checkpoint; no substitution is permitted"
            )
        return spec.factory()
