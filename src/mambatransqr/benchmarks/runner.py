...
class BenchmarkRunner:
    ...
    def run(
        ...
        *,
        model_name: str = "Mamba-TransQR",
    ) -> list[dict[str, Any]]:
        root = Path(self.config.output)
        identity = reproducibility_metadata(
            {"seed": self.config.seed}, self.config.manifest, model
        )
        ...
        for batch in batches:
            ...
            row = {
                "model": model_name,
                **image_metrics(output, target),
                **measure(model, inputs, self.config.iterations),
                **{
                    key: batch.get(key)
                    for key in (
                        "degradation_type",
        ...
        for stem, keys in (
            ("model_summary", ("model",)),
            ("degradation_summary", ("degradation_type",)),
            ("severity_summary", ("degradation_severity",)),
            ("decode_summary", ("model",)),
            ("latency_summary", ("model",)),
        ):
            export_table(aggregate(rows, keys), stem, root)
        return rows
