...
def export_table(rows: list[dict[str, Any]], stem: str, directory: Path) -> None:
    ...
    directory.mkdir(parents=True, exist_ok=True); columns = sorted({key for row in rows for key in row})
    ...
    markdown = ["| " + " | ".join(columns) + " |", "|" + "---|" * len(columns)] + ["| " + " | ".join(str(row.get(key, "")) for key in columns) + " |" for row in rows]
    (directory / f"{stem}.md").write_text("\n".join(markdown) + "\n", encoding="utf-8")
    latex = ["\\begin{tabular}{" + "l" * len(columns) + "}", " & ".join(columns) + " \\\\ \\hline"] + [" & ".join(str(row.get(key, "")) for key in columns) + " \\\\" for row in rows] + ["\\end{tabular}"]
