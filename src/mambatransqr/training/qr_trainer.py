...
class QRRestorationTrainer(Trainer):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        super().__init__(
            model, optimizer, LossManager({"qr": objective}), config, **kwargs
        )
    ...
    def train_batch(self, batch: Mapping[str, Any]) -> dict[str, float]:
        ...
        with self.engine.amp.autocast():
            ...
            loss = self.objective(output, targets, metadata)
        self.engine.amp.backward(loss)
        self.engine.amp.step(self.optimizer)
        ...
        return {
            ...
            **{
                ...
                for name, value in self.objective.last_components.items()
            },
        }
    ...
    def validate_qr(self, loader: Iterable[Mapping[str, Any]]) -> dict[str, float]:
        ...
        for batch in loader:
            ...
            totals["val_loss"] += float(self.objective(output, targets, metadata))
            totals["val_psnr"] += float(-10 * torch.log10(mse.clamp_min(1e-8)))
            totals["val_ssim"] += float(1 - self.objective.ssim(output, targets))
            totals["val_decode_rate"] += float(
                ((output - 0.5).abs() > 0.25).float().mean()
            )
            count += 1
