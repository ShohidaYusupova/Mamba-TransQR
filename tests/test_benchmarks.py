...
from mambatransqr.benchmarks.registry import BaselineRegistry, BaselineSpec  # noqa: E402
...
def test_registry_rejects_unregistered_substitution() -> None:
    ...
    with pytest.raises(KeyError): registry.build("SRCNN")
    registry.register(BaselineSpec("SRCNN", factory=torch.nn.Identity, provenance="local test"))
    assert isinstance(registry.build("SRCNN"), torch.nn.Identity)
def test_aggregation_and_table_exports(tmp_path: Path) -> None:
    rows = aggregate([{"model": "x", "psnr": 20.0}, {"model": "x", "psnr": 22.0}], ("model",))
